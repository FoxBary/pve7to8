...
# GitHub 反馈 / GitHub Issues: https://github.com/FoxBary/pve7to8
...
check_status() {
    if [ $? -eq 0 ]; then
        ...
        echo "反馈问题至 / Report issues to: https://github.com/FoxBary/pve7to8" | tee -a "$LOG_FILE"
        exit 1
    fi
}
...
# 步骤 1：配置 Debian 12 (Bookworm) 和 Proxmox VE 社区源 / Step 1: Configure Debian 12 (Bookworm) and Proxmox VE community sources
print_step 1 "配置 Debian 12 (Bookworm) 和 Proxmox VE 社区源 / Configuring Debian 12 (Bookworm) and Proxmox VE community sources"
...
# 步骤 2：验证 root 权限 / Step 2: Verify root privileges
print_step 2 "验证 root 权限 / Verifying root privileges"
if [ "$(id -u)" -ne 0 ]; then
...
# 步骤 3：检查磁盘空间 / Step 3: Check disk space
print_step 3 "检查磁盘空间（需至少 5GB） / Checking disk space (requires at least 5GB)"
...
# 步骤 4：测试网络连接 / Step 4: Test network connectivity
print_step 4 "测试网络连接 / Testing network connectivity"
...
# 步骤 5：检查当前 Proxmox VE 版本 / Step 5: Check current Proxmox VE version
print_step 5 "检查当前 Proxmox VE 版本 / Checking current Proxmox VE version"
...
# 步骤 6：显示硬件信息 / Step 6: Display hardware information
print_step 6 "显示硬件信息 / Displaying hardware information"
...
# 步骤 7：备份关键配置文件 / Step 7: Backup critical configuration files
print_step 7 "备份关键配置文件 / Backing up critical configuration files"
mkdir -p "$BACKUP_DIR"
...
# 步骤 8：更新 Proxmox VE 7.4 到最新补丁 / Step 8: Update Proxmox VE 7.4 to latest patch
print_step 8 "更新 Proxmox VE 7.4 到最新补丁 / Updating Proxmox VE 7.4 to latest patch"
...
# 步骤 9：安装常用工具 / Step 9: Install common tools
print_step 9 "安装常用工具 / Installing common tools"
...
# 步骤 10：运行 Proxmox VE 7 到 8 升级检查 / Step 10: Run Proxmox VE 7 to 8 upgrade checker
print_step 10 "运行 Proxmox VE 7 到 8 升级检查 / Running Proxmox VE 7 to 8 upgrade checker"
...
# 步骤 11：执行系统升级到 Proxmox VE 8 / Step 11: Perform system upgrade to Proxmox VE 8
print_step 11 "执行系统升级到 Proxmox VE 8 / Performing system upgrade to Proxmox VE 8"
...
# 步骤 12：安装 systemd-boot（若使用 UEFI 和 ZFS） / Step 12: Install systemd-boot (if using UEFI and ZFS)
print_step 12 "检查并安装 systemd-boot（若适用） / Checking and installing systemd-boot (if applicable)"
...
# 步骤 13：清理 APT 缓存 / Step 13: Clean APT cache
print_step 13 "清理 APT 缓存 / Cleaning APT cache"
apt clean
check_status "APT 缓存清理 / APT cache cleanup"
echo "APT 缓存已清理 / APT cache cleaned." | tee -a "$LOG_FILE"

# 步骤 14：清理旧内核（可选） / Step 14: Clean old kernels (optional)
print_step 14 "清理旧内核（可选） / Cleaning old kernels (optional)"
apt autoremove --purge -y
check_status "旧内核清理 / Old kernels cleanup"
echo "旧内核已清理 / Old kernels cleaned." | tee -a "$LOG_FILE"

# 步骤 15：重启系统 / Step 15: Reboot system
print_step 15 "重启系统以应用更改 / Rebooting system to apply changes"
...
如有问题，请提交 issue 到 / For issues, submit to: https://github.com/FoxBary/pve7to8
==============================================================
EOF
